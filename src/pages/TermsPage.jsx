...
import { defaultTermsContent } from '../data/defaults';
...
export default function TermsPage() {
  const { data: pageData, loading } = useLegalPage('terms');

  const content = pageData?.content || null;
  const title = pageData?.title || '이용약관';
  ...
  return (
    <>
      <SEO 
        title="이용약관"
        description="24시 코리아 부동산의 서비스 이용약관입니다."
      />
          ...
          <Card padding="lg">
            {loading ? (
              ...
              <div className="prose prose-slate max-w-none">
                {defaultTermsContent.map((section, index) => (
